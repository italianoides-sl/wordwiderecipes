...
export default function ContentGrid({ locale, rows }: { locale: string; rows: Content[] }) {
  ...
  return (
    <section className="directory-grid" aria-label="Articulos publicados">
      {rows.map((item) => (
        <a className="directory-card" href={contentHref(locale, item)} key={item.id}>
