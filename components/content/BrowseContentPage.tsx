    ...
    <main className="wwr-page directory-page">
      ...
      {hasFilters && !result.results.length ? (
        <section className="bp-empty">
          ...
          <p>Prueba a quitar uno o explora <a href={`/${locale}/recipes`}>todas las recetas</a>.</p>
        </section>
        ...
        <ContentGrid locale={locale} rows={result.results} />
      )}

      {(page > 0 || result.hasMore) ? (
        <nav className="pagination-nav" aria-label="Paginación">
          {page > 0 ? <a href={`${filterHref(locale, filters)}?page=${page - 1}`}>← Anterior</a> : null}
          {result.hasMore ? <a href={`${filterHref(locale, filters)}?page=${page + 1}`}>Siguiente →</a> : null}
        </nav>
      ) : null}
    </main>
  );
