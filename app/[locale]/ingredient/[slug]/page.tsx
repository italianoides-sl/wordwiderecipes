...
export async function generateMetadata({ params }: { params: { locale: string; slug: string } }) {
  const content = await getContentBySlugAndType(params.slug, params.locale as Locale, 'ingredient').catch(() => null);
  if (!content) return {};
  return buildMetadata(content);
}

export async function generateStaticParams() {
  const rows = await getPublishedSlugsByType('ingredient').catch(() => []);
  return rows.map((row) => ({ locale: row.locale, slug: row.slug }));
}

export default async function IngredientPage({ params }: { params: { locale: string; slug: string } }) {
  const content = await getContentBySlugAndType(params.slug, params.locale as Locale, 'ingredient').catch(() => null);
  if (!content) notFound();
