...
export async function generateMetadata({ params }: { params: { locale: string; slug: string } }) {
  const content = await getContentBySlugAndType(params.slug, params.locale as Locale, 'technique').catch(() => null);
  if (!content) return {};
  return buildMetadata(content);
}

export async function generateStaticParams() {
  const rows = await getPublishedSlugsByType('technique').catch(() => []);
