...
export async function generateMetadata({ params }: { params: { locale: string; slug: string } }) {
  const content = await getContentBySlugAndType(params.slug, params.locale as Locale, 'spice').catch(() => null);
  if (!content) return {};
  return buildMetadata(content);
}

export async function generateStaticParams() {
  const rows = await getPublishedSlugsByType('spice').catch(() => []);
  return rows.map((row) => ({ locale: row.locale, slug: row.slug }));
}

export default async function SpicePage({ params }: { params: { locale: string; slug: string } }) {
  const content = await getContentBySlugAndType(params.slug, params.locale as Locale, 'spice').catch(() => null);
  if (!content) notFound();
